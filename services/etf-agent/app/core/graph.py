"""LangGraph StateGraph for the etf-agent 8-phase decision loop.
...
# Prometheus metrics
# ---------------------------------------------------------------------------
etf_agent_decisions_total = Counter(
    "etf_agent_decisions_total",
    "Total ETF agent decisions",
    ["asset", "action"],
)
etf_agent_phase_total = Counter(
    "etf_agent_phase_total",
    "Total ETF agent phase executions",
    ["phase", "status"],
)
...
def gather_node(state: AgentState) -> dict:
    ...
    try:
        ...
        etf_agent_phase_total.labels(phase="gather", status="error").inc()
        return {
            "signal": None,
    ...
    if age > SIGNAL_STALENESS_SECONDS:
        ...
        etf_agent_phase_total.labels(phase="gather", status="stale").inc()
        return {
            "signal": signal_dict,
    ...
    etf_agent_phase_total.labels(phase="gather", status="ok").inc()
    return {
        "signal": signal_dict,
        "signal_age_seconds": age,
        "phase_timings": timings,
...
def detect_node(state: AgentState) -> dict:
    ...
    etf_agent_phase_total.labels(phase="detect", status="ok").inc()
...
def recall_node(state: AgentState) -> dict:
    ...
    etf_agent_phase_total.labels(phase="recall", status="ok").inc()
...
def select_node(state: AgentState) -> dict:
    ...
    strategy_dict = None
    try:
        params = {"asset_type": "etf", "status": "ACTIVE"}
        if user_id:
    ...
    etf_agent_phase_total.labels(phase="select", status="ok").inc()
...
def score_node(state: AgentState) -> dict:
    ...
    etf_agent_phase_total.labels(phase="score", status="ok").inc()
...
def check_node(state: AgentState) -> dict:
    ...
    etf_agent_phase_total.labels(phase="check", status="ok").inc()
...
def execute_node(state: AgentState) -> dict:
    ...
    etf_agent_phase_total.labels(phase="execute", status="ok").inc()
...
def record_node(state: AgentState) -> dict:
    ...
    etf_agent_decisions_total.labels(asset=asset, action=action).inc()
    ...
    etf_agent_phase_total.labels(phase="record", status="ok").inc()
