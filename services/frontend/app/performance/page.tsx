...
function PerformanceContent() {
  ...
  if (loading) {
    return <main className="grid gap-6">{[0,1].map(i => <div key={i} className="rounded border border-neutral-200 bg-white p-6 skeleton h-40" />)}</main>;
  }
    ...
    <PageTransition>
      <main className="grid gap-6">
        <section className="rounded border border-neutral-200 bg-white p-6">
          <p className="text-sm font-medium uppercase tracking-wider text-neutral-400">PERFORMANCE</p>
          <h2 className="mt-1 text-2xl font-semibold text-neutral-900">에이전트 성과</h2>
            ...
            {[
              ...
              <StaggerItem key={i}>
                <div className="rounded border border-neutral-200 bg-white p-5">
                  ...
                  <p className={`mt-1 font-mono text-2xl font-semibold ${m.color ? m.color(m.value ?? 0) : "text-neutral-900"}`}>
        ...
        <FadeInView delay={0.1}>
          <section className="rounded border border-neutral-200 bg-white p-6">
            ...
            <h3 className="mt-2 text-lg font-semibold text-neutral-900">포트폴리오 상세</h3>
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              <div className="rounded border border-neutral-200 bg-white p-4">
                <p className="text-xs font-medium uppercase tracking-wider text-neutral-400">총 노출</p>
                <p className="mt-1 font-mono text-2xl font-semibold text-neutral-900">${portfolio?.total_exposure?.toLocaleString() ?? "0"}</p>
              </div>
              <div className="rounded border border-neutral-200 bg-white p-4">
              ...
              <div className="rounded border border-neutral-200 bg-white p-4">
                <p className="text-xs font-medium uppercase tracking-wider text-neutral-400">실현 PnL</p>
                <p className={`mt-1 font-mono text-2xl font-semibold ${(portfolio?.realized_pnl ?? 0) >= 0 ? "text-green-600" : "text-red-600"}`}>
                  ...
                  {Object.entries(portfolio.concentration).map(([asset, weight]: [string, any]) => (
                    <div key={asset} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 rounded-full bg-neutral-100 h-1.5">
                        <div className="rounded-full bg-neutral-900 h-1.5" style={{ width: `${Math.min(weight * 100, 100)}%` }} />
                      </div>
                      <span className="font-mono text-xs text-neutral-400">{(weight * 100).toFixed(1)}%</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {stats?.recent_trade_pnls && stats.recent_trade_pnls.length > 0 && (
          <FadeInView delay={0.15}>
            <section className="rounded border border-neutral-200 bg-white p-6">
              <p className="text-sm font-medium uppercase tracking-wider text-neutral-400">RECENT PNL</p>
              <h3 className="mt-2 text-lg font-semibold text-neutral-900">최근 거래 PnL</h3>
                ...
                {stats.recent_trade_pnls.map((pnl: number, i: number) => {
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center justify-end h-full">
                      <div
                        className={`w-full rounded-t ${pnl >= 0 ? "bg-green-600" : "bg-red-500"}`}
